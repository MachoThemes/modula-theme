...
// remove child licenses
     ...
     *
	 * */
	function modula_remove_version() {
		return '';
	}

	add_filter( 'the_generator', 'modula_remove_version' );
	...
	function modula_cleanup_query_string( $src ) {
		$parts = explode( '?', $src );

		return $parts[0];
	}

	add_filter( 'script_loader_src', 'modula_cleanup_query_string', 15, 1 );
	add_filter( 'style_loader_src', 'modula_cleanup_query_string', 15, 1 );
